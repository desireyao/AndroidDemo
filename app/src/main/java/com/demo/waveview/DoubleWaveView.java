...
public class DoubleWaveView extends View {
    ...
    private static  int TRANSLATE_X_SPEED_THR ;    // 第二条水波的移动速度 单位dp

    private int mXOffsetSpeedOne;//单位 PX
    private int mXOffsetSpeedTwo;//单位 PX
    private int mXOffsetSpeedThr;//单位 PX
    ...
    private int mXThrOffset;//第二条波浪的X轴 偏移量
    ...
    public DoubleWaveView(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            }else if(attr == R.styleable.DoubleWaveView_speedThr){
                TRANSLATE_X_SPEED_THR = a.getInteger(attr,10);
        ...
        mXOffsetSpeedThr = DensityUtil.dip2px(context, TRANSLATE_X_SPEED_THR);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(int i=0,j=0,k=0,l=0;i<mTotalWidth;i++){
            ...
            if(i+mXThrOffset<mTotalWidth){//第二条波纹图形绘制
                canvas.drawLine(i,mTotalHeight-mYPositions[mXThrOffset+i]-WaveHeight,i,mTotalHeight,mWavePaint);
            }else {//大于周期值，则设置为k(与相位相关，已移动的X距离)
                canvas.drawLine(i,mTotalHeight-mYPositions[l]-WaveHeight,i,mTotalHeight,mWavePaint);
                l++;
            }
        }
